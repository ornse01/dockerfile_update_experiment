...
*   **処理内容**:
    ...
    3.  修正可能な脆弱性が検出された（ビルドが失敗した）場合のみ、自動で更新スクリプトを走らせて `ARG SECURITY_UPDATE_DATE` を更新（Docker キャッシュを無効化して次回ビルドで最新パッチを適用）し、CHANGELOG に記録を追記して、再ビルド用のセキュリティ更新 PR を作成します。
