...
*   **処理内容**:
    1.  GHCR にある `latest` イメージに対して Trivy スキャンを厳格に実行します。検証しやすくするため、**全重大度 (`UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL`)** かつ **未修正の脆弱性 (`ignore-unfixed: false`)** をスキャンします。
    2.  脆弱性が検出された場合、自動で更新スクリプトを走らせてベースイメージを最新のダイジェストに更新し、CHANGELOG に記録を追記して、自動更新の PR を作成します。
